...
BG_PRESETS = [
    {"id": "default", "name": "Тёмный (по умолчанию)", "color": "#12121a"},
    {"id": "navy", "name": "Тёмно-синий", "color": "#0d1b2a"},
    {"id": "forest", "name": "Тёмно-зелёный", "color": "#0f1f14"},
    {"id": "wine", "name": "Тёмно-бордовый", "color": "#1f0d14"},
    {"id": "purple", "name": "Тёмно-фиолетовый", "color": "#160d2a"},
]
...
def default_state():
    return {
        ...
        "settings_bg_color": "#12121a",
        "settings_compact_numbers": False,
        "settings_bg_color": "#12121a",
        "settings_compact_numbers": False,
    }
...
def build_game(page: ft.Page, username="guest", uid=None, id_token=None):
    ...
    page.bgcolor = state.get("settings_bg_color", "#12121a")
    ...
    def format_number(n):
        n = int(n)
        if not state.get("settings_compact_numbers", False):
            return str(n)
    ...
    def refresh_top():
        points_text.value = format_number(state['points'])
    ...
    # ---------- (навигация теперь через ☰ меню, см. блок сборки экрана ниже) ----------
    ...
    def select_bg(preset):
        def handler(e):
            state["settings_bg_color"] = preset["color"]
            save_state()
            apply_bg_color()
            render_settings()
            page.update()
    ...
    def toggle_compact(e):
        state["settings_compact_numbers"] = not state.get("settings_compact_numbers", False)
        save_state()
        refresh_top()
        render_settings()

    settings_column = ft.Column(spacing=8, scroll=ft.ScrollMode.AUTO)

    def render_settings():
        settings_column.controls.clear()
        settings_column.controls.append(ft.Text("Фон игры", size=14, weight=ft.FontWeight.BOLD, color="white"))
        for preset in BG_PRESETS:
            selected = state.get("settings_bg_color", "#12121a") == preset["color"]
            settings_column.controls.append(
                ft.Container(
                    padding=10, border_radius=10, bgcolor="#1e1e2a",
                    content=ft.Row(
                        alignment=ft.MainAxisAlignment.SPACE_BETWEEN,
                        controls=[
                            ft.Row(controls=[
                                ft.Container(width=20, height=20, bgcolor=preset["color"], border_radius=6, border=ft.border.all(1, "#555")),
                                ft.Text(preset["name"], size=13, color="white"),
                            ]),
                            ft.Text("✅ Выбран", size=12, color="#66bb6a") if selected else ft.ElevatedButton("Выбрать", on_click=select_bg(preset), bgcolor="#3a3a45", color="white"),
                        ],
                    ),
                )
            )
        settings_column.controls.append(ft.Divider(color="#2a2a35"))
        settings_column.controls.append(
            ft.Container(
                padding=10, border_radius=10, bgcolor="#1e1e2a",
                content=ft.Row(alignment=ft.MainAxisAlignment.SPACE_BETWEEN, controls=[
                    ft.Text("Сокращать большие числа (1.2K)", size=13, color="white"),
                    ft.Switch(value=state.get("settings_compact_numbers", False), on_change=toggle_compact),
                ]),
            )
        )
        settings_column.controls.append(ft.Divider(color="#2a2a35"))
        settings_column.controls.append(ft.Text(f"Аккаунт: {username}", size=13, color="#9e9e9e"))
        settings_column.controls.append(ft.ElevatedButton("🚪 Выйти из аккаунта", on_click=do_logout, bgcolor="#3a3a45", color="white"))
        settings_column.controls.append(ft.ElevatedButton("🗑 Сбросить прогресс", on_click=confirm_reset, bgcolor="#5c2b2b", color="white"))
        page.update()
    ...
    # ---------- Сборка экрана: главный (тапалка) + разделы через меню ----------
    ...
    SECTIONS = [
        ("shop", "🛒 Магазин", shop_column, render_shop),
        ("quests", "📋 Задания", quests_column, render_quests),
        ("skins", "🎨 Скины", skins_column, render_skins),
        ("cases", "🎁 Кейсы", cases_column, render_cases),
        ("stats", "📊 Статистика", stats_column, render_stats),
        ("leaderboard", "🏆 Таблица лидеров", leaderboard_column, render_leaderboard),
    ]
    if username == ADMIN_USERNAME:
        SECTIONS.append(("admin", "🛠 Админ-панель", admin_column, render_admin))
    SECTIONS.append(("settings", "⚙️ Настройки", settings_column, render_settings))
    ...
    def render_view(key):
        page.controls.clear()
        if key == "home":
            page.add(build_home_column())
        else:
            title, column, render_fn = next((s[1], s[2], s[3]) for s in SECTIONS if s[0] == key)
            render_fn()
            page.add(
                ft.Column(
                    horizontal_alignment=ft.CrossAxisAlignment.CENTER,
                    ...
                    controls=[
                        ...
                        ft.Row(
                            controls=[
                                ft.IconButton(icon=ft.Icons.ARROW_BACK, icon_color="white", on_click=go_home),
                                ft.Text(title, size=18, weight=ft.FontWeight.BOLD, color="white"),
                            ],
                        ),
                        ft.Container(content=column, expand=True, padding=10, width=340),
                    ],
                )
            )
        page.update()
    ...
    def open_nav_menu(e):
        ...
        items = [nav_button(s[0], s[1]) for s in SECTIONS]
        page.dialog = ft.AlertDialog(
            title=ft.Text("Меню"),
            content=ft.Column(spacing=2, controls=items, tight=True),
        )
        page.dialog.open = True
        page.update()
    ...
    header_row = ft.Row(
        alignment=ft.MainAxisAlignment.SPACE_BETWEEN,
        controls=[
            ft.IconButton(icon=ft.Icons.MENU, icon_color="white", on_click=open_nav_menu),
            ft.Column(spacing=0, horizontal_alignment=ft.CrossAxisAlignment.CENTER, controls=[league_text, user_label]),
